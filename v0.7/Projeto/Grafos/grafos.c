...
Node criarNode(int ID, char* Cidade) {
...
void escreverVerticesBinario(Node grafo) {
    ...
    while (node != NULL) {
        ...
        int tamanhoCidade = strlen(node->Cidade) + 1; // Inclui o caractere nulo
        fwrite(&tamanhoCidade, sizeof(int), 1, arquivo);
        fwrite(node->Cidade, sizeof(char), tamanhoCidade, arquivo);
        node = node->seguinte;
    }
...
void lerVerticesBinario(Node* grafo) {
    ...
    Node novoNode, ultimoNode = NULL;
    int ID, tamanhoCidade;
    char Cidade[100];

    while (fread(&ID, sizeof(int), 1, arquivo) == 1) {
        fread(&tamanhoCidade, sizeof(int), 1, arquivo);
        fread(Cidade, sizeof(char), tamanhoCidade, arquivo);
        Cidade[tamanhoCidade - 1] = '\0';
        novoNode = criarNode(ID, Cidade);
...
void buscarVerticesAlcance(Node grafo, int verticeInicial, int distanciaLimite) {
    // Inicialização das estruturas de dados
    int* visitado = (int*)malloc(sizeof(int) * MAX_VERTICES); // MAX_VERTICES é o número máximo de vértices no seu grafo
    memset(visitado, 0, sizeof(int) * MAX_VERTICES);

    int* distancia = (int*)malloc(sizeof(int) * MAX_VERTICES);
    memset(distancia, -1, sizeof(int) * MAX_VERTICES);

    Node* fila = (Node*)malloc(sizeof(Node) * MAX_VERTICES);
    int frente = 0, tras = 0;
    ...
    // BFS adaptado para calcular a distância acumulada
    while (frente != tras) {
        ...
        while (adjacente != NULL) {
            ...
            if (!visitado[noAdjacente->ID]) {
                ...
                distancia[noAdjacente->ID] = distancia[noAtual->ID] + peso;
